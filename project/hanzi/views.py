...
def stroke_ajax_search(request):
	...
	if(order == '1'):
		...
		if(check_mode==1):
			...
			pages = total / page_size + 1
			result = HanziSet.objects.filter( Q(max_strokes__lte=max_num)&Q(max_strokes__gte=min_num)&Q(structure__contains=structure)&Q(stroke_serial__regex=parts_rex)&Q(similar_parts__regex=similar_parts_rex) ).order_by('source')[(page_num-1)*page_size : page_num*page_size]

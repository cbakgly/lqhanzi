...
alter table lq_tasks rename lq_tasks_bk;
...
alter table lq_tasks rename lq_tasks_orig;
alter table lq_tasks_bk rename lq_tasks;
...
insert into tmp (business_type,content_id) select 5,id from django_content_type where app_label="backend" and model="interdictdedup";
insert into tmp (business_type,content_id) select 9,id from django_content_type where app_label="backend" and model="inputpage";
...
select user_id, task_package_id, page_num, completed_at, completed_at, c_t, u_t, content_type_id, 9, 3, 3, 0, "" from tmp_page_num;
